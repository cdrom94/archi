...
export const PlacePage = () => {
	let { url } = useParams();
	const place: ArchitectureData = architectureList.find(place => place.url === url);
	...
	return (
		<motion.div initial='initial' animate='animate' exit='exit' className='container'>
			<div className='info'>
				...
				<motion.h4
					initial={{ opacity: 0 }}
					animate={{ opacity: 1, y: 20, transition: { delay: 0.1, ...animate } }}
					exit={{ opacity: 0, y: -20, transition: { delay: 0.1, ...animate } }}
				>
					{place.name}
				</motion.h4>
				<motion.h5
					initial={{ opacity: 0 }}
					animate={{ opacity: 1, y: 20, transition: { delay: 0.3, ...animate } }}
					exit={{ opacity: 0, y: -20, transition: { delay: 0.3, ...animate } }}
				>
					{place.location}
				</motion.h5>
				<motion.div
					initial={{ opacity: 0 }}
					animate={{ opacity: 1, y: 20, transition: { delay: 0.5, ...animate } }}
			...
			<motion.div
				...
				animate={{ opacity: 1, y: 0, transition: { ...animate } }}
				exit={{ opacity: 0, y: 200, transition: { ...animate } }}
				className='images'
			>
